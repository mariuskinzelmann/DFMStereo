...
class hourglass(nn.Module):
    def __init__(self, in_channels, dims):
        ...
        self.conv3_up = FasterResidualBlock_32_16(in_channels*6, in_channels*4, feat_channels=dims[2])

        self.conv2_up = FasterResidualBlock_16_08(in_channels*4, in_channels*2, feat_channels=dims[1])
...
class DFMStereo_Medium(nn.Module):
...
def init_dfmstereo_medium(args=None, eval=True):
    checkpoint_path = Path(__file__).parent.parent / "pretrained" / f"dfmstereo_medium_{args.dfmstereo_ckpt}.pth" if args is not None and hasattr(args, "dfmstereo_ckpt") else None
