...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args):
        ...
        if hasattr(args, "cor_planes"):
            cor_planes = args.cor_planes #args.corr_levels * (2*args.corr_radius + 1) * args.volume_dim
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim=96):
        ...
        if hasattr(args, "hidden_state") and self.args.hidden_state:
            self.trans_1x1 = nn.Conv2d(hidden_dim, 128, kernel_size=1, stride=1, padding=0)

    def forward(self, net, inp, corr=None, disp=None, itr=-1, being_taught=False):
        update_block_outputs = {}

        motion_features = self.encoder(disp, corr)
        net = self.gru(net, inp, motion_features)

        if being_taught and self.args.hidden_state and itr in self.args.update_block_iters:
            update_block_outputs[f"iter_{itr}_hidden_state"] = self.trans_1x1(net)

        delta_disp = self.disp_head(net)
        mask_feat_4 = self.mask_feat_4[0](net)
        if being_taught and self.args.mask and itr in self.args.update_block_iters:
            update_block_outputs[f"iter_{itr}_mask"] = mask_feat_4
        mask_feat_4 = self.mask_feat_4[1](mask_feat_4)

        return net, mask_feat_4, delta_disp, update_block_outputs
