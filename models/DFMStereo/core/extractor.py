...
class Feature(SubModule):
    def __init__(self):
        ...
        self.d_out = [chans[1]*2, chans[2]*2, chans[3]*2, chans[4]]
...
class Feature_EdgeNext(SubModule):
    def __init__(self):
        super().__init__()
        # --- EdgeNext ---
        model = timm.create_model('edgenext_small.usi_in1k', pretrained=True, features_only=False)
        ...
        self.conv4 = BasicConv_IN(chans[0]*2, chans[0]*2, kernel_size=3, stride=1, padding=1)

        self.d_out = [chans[0]*2, chans[1]*2, chans[2]*2, chans[3]]

    def forward(self, x):
        x = self.stem(x)
        x4 = self.stages[0](x) # 48 Channels
        x8 = self.stages[1](x4) # 96 Channels
        x16 = self.stages[2](x8) # 160 Channels
        x32 = self.stages[3](x16) # 304 Channels        

        x16 = self.deconv32_16(x32, x16) # 320 Channels
        x8 = self.deconv16_8(x16, x8) # 192 Channels
        x4 = self.deconv8_4(x8, x4) # 96 Channels for Student vs 224=96+128 Channels for Teacher
        x4 = self.conv4(x4)

        return [x4, x8, x16, x32]
    
class Feature_MobileNetV2(SubModule):
    def __init__(self):
        super(Feature_MobileNetV2, self).__init__()
        pretrained =  True
        model = timm.create_model('mobilenetv2_100', pretrained=pretrained, features_only=True)
        layers = [1,2,3,5,6]
        chans = [16, 24, 32, 96, 160]
