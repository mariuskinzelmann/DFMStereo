...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args, ngroup=8):
        ...
        self.convd1 = nn.Conv2d(1, 64, 7, padding=3) #TODO Consider placing distillation point further inside
        self.convd2 = nn.Conv2d(64, 64, 3, padding=1)
        self.conv = nn.Conv2d(64+256, 128-1, 3, padding=1)

    def forward(self, disp, corr, itr=-1):
        ...
        out = self.conv(cor_disp)

        if hasattr(self.args, "motion_features") and self.args.motion_features and itr in [0,1,3,7]:
            motion_features = out.detach()
            out = F.relu(out)
            return torch.cat([out, disp], dim=1), motion_features
...
class BasicSelectiveMultiUpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, corr, disp, att, itr=-1):
        update_block_outputs = {}
        ...
        motion_features = self.encoder(disp, corr, itr)
        #if hasattr(self.args, "motion_features") and self.args.motion_features  and itr in [0,1,3,7]:
        #    motion_features, update_block_outputs[f"iter_{itr}_motion_features"] = motion_features # motion_features for kd are pre_relu and without disp concatenated
        ...
        # --- 1/4 Res Hidden State ---
        if self.args.hidden_state and itr in self.args.update_block_iters:
            update_block_outputs[f"iter_{itr}_hidden_state"] = net[0].detach()

        delta_disp = self.disp_head(net[0])
        
        # --- Mask ----
        mask = self.mask[0:3](net[0])
        if self.args.mask and itr in self.args.update_block_iters:
            update_block_outputs[f"iter_{itr}_mask"] = mask.detach()
        mask = .25 * self.mask[3](mask) # scale mask to balence gradients

        return net, mask, delta_disp, update_block_outputs
