...
from models.FastFoundationStereo.core.update import BasicSelectiveMultiUpdateBlock
from models.FastFoundationStereo.core.extractor import ContextNetSharedBackbone, Feature
from models.FastFoundationStereo.core.geometry import Combined_Geo_Encoding_Volume
from models.FastFoundationStereo.core.submodule import (
...
from models.DFMStereo.core.submodule import build_gwc_volume, build_concat_volume
from models.FastFoundationStereo.core.utils.utils import InputPadder

sys.modules['foundation_stereo_ori'] = sys.modules['core']
sys.modules['foundation_stereo_ori.submodule'] = sys.modules['models.FastFoundationStereo.core.submodule']
#sys.modules['foundation_stereo_ori.extractor'] = sys.modules['models.FastFoundationStereo.core.extractor']
sys.modules['foundation_stereo_ori.update'] = sys.modules['models.FastFoundationStereo.core.update']
#sys.modules['foundation_stereo_ori.foundation_stereo'] = sys.modules['models.FastFoundationStereo.core.foundation_stereo'


import models.FastFoundationStereo.core.update as update_mod
sys.modules['foundation_stereo_ori.update'] = update_mod

import models.FastFoundationStereo.core.foundation_stereo as foundation_mod
sys.modules['core.foundation_stereo'] = foundation_mod

import models.FastFoundationStereo.core.extractor as extractor_mod
sys.modules['foundation_stereo_ori.extractor'] = extractor_mod
...
class hourglass(nn.Module):
    ...
    def forward(self, x, features):
        ...
        if not hasattr(self, 'post32_to_16') or self.post32_to_16 is None:
          ...
          # ResNet-Style Block:
          # 1) BasicConv(kernel_size=4, stride=4) + Disparity Transformer -> x_delta
          # 2) x + x_delta -> BasicConv + BasicResNetBlock -> Out
          conv2 = self.post32_to_16(conv2, conv3, features[2]) # ResNet-Style Block
        ...
        if not hasattr(self, 'post16_to_8') or self.post16_to_8 is None:
          ...
          # 1) BasicConv(kernel_size=4, stride=4) + Disparity Transformer -> x_delta
          # 2) x + x_delta -> BasicConv + BasicResNetBlock -> Out
          conv1 = self.post16_to_8(conv1, conv2, features[1]) # ResNet-Style Block

        conv = self.conv1_up(conv1)
        if not hasattr(self, 'post8_to_4') or self.post8_to_4 is None:
          x = self.conv_patch(x)
          x = self.atts["4"](x)
          hg_outputs["agg_cost_volume_hg"] = conv.detach()
          hg_outputs["agg_cost_volume_dt"] = x.detach()
...
class FastFoundationStereo(nn.Module):
  ...
  def upsample_disp(self, disp, mask_feat_4, stem_2x):
    with torch.amp.autocast('cuda', enabled=self.args.mixed_precision, dtype=torch.float16):
  ...
  def forward(self, image1, image2, iters=12, test_mode=False, teach_mode=False, low_memory=False, init_disp=None, profile=False, optimize_build_volume='pytorch1', being_taught=False):
    ...
    with torch.amp.autocast('cuda', enabled=self.args.mixed_precision, dtype=torch.float16):
      ...
      if teach_mode or being_taught and self.args.features:
        resolutions = ['4x', '8x', '16x', '32x']
        for res, feat_left, feat_right in zip(resolutions, features_left, features_right):
          teacher_outputs[f"feat_{res}_left"] = feat_left.detach()
          teacher_outputs[f"feat_{res}_right"] = feat_right.detach()
      ...
      elif optimize_build_volume=='slow':
      ...
      # --- This is intended for Checkpoint 23-36-37 ---
      comb_volume = self.corr_stem(comb_volume) # nn.Identity for some reason
      if teach_mode or being_taught and self.args.cost_volume:
        comb_volume = self.corr_feature_att.layers[0](comb_volume)
        comb_volume = self.corr_feature_att.layers[1](comb_volume)
        teacher_outputs["cost_volume"] = comb_volume.detach()
        comb_volume = self.corr_feature_att.layers[2](comb_volume, features_left[0])
      else:
        comb_volume = self.corr_feature_att(comb_volume, features_left[0])

      comb_volume = self.cost_agg(comb_volume, features_left)

      if teach_mode or being_taught and self.args.agg_cost_volume:
        teacher_outputs["agg_cost_volume"] = comb_volume.detach()

      # Init disp from geometry encoding volume
      logits = self.classifier(comb_volume).squeeze(1)

      if teach_mode or being_taught and self.args.init_disp_logits:
        teacher_outputs["init_disp_logits"] = logits.detach()
      ...
      if teach_mode or being_taught and self.args.init_hidden_state:
    ...
    for itr in range(iters):
      ...
      with torch.amp.autocast('cuda', enabled=self.args.mixed_precision, dtype=torch.float16):
        net_list, mask_feat_4, delta_disp, update_block_outputs = self.update_block.forward(net_list, inp_list, geo_feat.to(self.dtype), disp, att, itr)

      if teach_mode or being_taught:
      ...
      if teach_mode or being_taught and self.args.refined_disp and itr in self.args.update_block_iters:
    ...
    init_disp_up = init_disp
    teacher_outputs["init_disp_up"] = [init_disp_up]
    teacher_outputs["disp_preds"] = disp_preds
...
def init_weights(m):
    ...
    if isinstance(m, (nn.Conv2d, nn.Conv3d, nn.ConvTranspose2d, nn.ConvTranspose3d)):
        # Kaiming Normal is standard for layers followed by ReLU
        nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        if m.bias is not None:
            nn.init.constant_(m.bias, 0)

    # --- 2D and 3D Normalization Layers ---
    elif isinstance(m, (nn.BatchNorm2d, nn.BatchNorm3d, nn.SyncBatchNorm, nn.GroupNorm, nn.LayerNorm)):
    ...
    # --- Embeddings (if the model uses Transformers/Feature tokens) ---
    elif isinstance(m, nn.Embedding):
        nn.init.normal_(m.weight, mean=0, std=0.02)

def init_fast_foundation_stereo(args, reset_weights=False):
  from pathlib import Path
  PATH = Path(__file__).parent.parent / "weights" / "23-36-37" / "model_best_bp2_serialize.pth"
  import models.FastFoundationStereo.core.extractor as extractor_mod
  import models.FastFoundationStereo.core.update as update_mod
  import models.FastFoundationStereo.core.submodule as submodule_mod
  import models.FastFoundationStereo.core.foundation_stereo as foundation_mod
  import models.FastFoundationStereo.core.geometry as geometry_mod
  import models.FastFoundationStereo.core.distill_block as distill_mod

  # Override ALL expected modules
  sys.modules['core.extractor'] = extractor_mod
  sys.modules['core.update'] = update_mod
  sys.modules['core.submodule'] = submodule_mod
  sys.modules['core.foundation_stereo'] = foundation_mod
  sys.modules['core.geometry'] = geometry_mod
  sys.modules['core.distill_block'] = distill_mod
  
  model = torch.load(PATH, map_location='cpu', weights_only=False)
  model.args.valid_iters = 12
  model.args.max_disp = 192

  model.eval()

  # This for Retraining this model from Scratch for Evaluation Purposes
  if reset_weights:
    model.apply(init_weights)

    # Load EdgeNext Checkpoint
    edge_next = timm.create_model('edgenext_small', pretrained=True, features_only=False)
    model.feature.stem = edge_next.stem
    model.feature.stages = edge_next.stages
  ...
  model.args.being_taught=args.being_taught if args and hasattr(args, "being_taught") else False
  model.args.features=args.features if args and hasattr(args, "features") else False
  model.args.factor_transfer=args.factor_transfer if args and hasattr(args, "factor_transfer") else False
  model.args.cost_volume=args.cost_volume if args and hasattr(args, "cost_volume") else False
  model.args.agg_cost_volume=args.agg_cost_volume if args and hasattr(args, "agg_cost_volume") else False
  model.args.init_disp_logits=args.init_disp_logits if args and hasattr(args, "init_disp_logits") else False
  model.args.init_hidden_state=args.init_hidden_state if args and hasattr(args, "init_hidden_state") else False
  model.args.update_block_iters=args.update_block_iters if args and hasattr(args, "update_block_iters") else []
  model.args.hidden_state=args.hidden_state if args and hasattr(args, "hidden_state") else False
  model.args.mask=args.mask if args and hasattr(args, "mask") else False
  model.args.refined_disp=args.refined_disp if args and hasattr(args, "refined_disp") else False
