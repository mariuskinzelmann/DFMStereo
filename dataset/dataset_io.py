...
def read(file):
    ...
    elif file.endswith('.pfm'): return readPFM(file)
...
def readImage(name):
    try:
        ...
        print(f"ERROR: Could not read image {name}, Exeception:{e}")
        raise ValueError
    img_rgb = img.convert('RGB')
    np_img = np.array(img_rgb, dtype=np.uint8)
    return np_img

def readDisparity_png(name):
    try:
        img = Image.open(name)
    except Exception as e:
        print(f"ERROR: Could not read disparity {name}, Exception: {e}")
        raise ValueError
